          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                />
                <button type="button" onClick={() => setShowPass(v => !v)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: T.muted, fontSize: 16, padding: 2, cursor: 'pointer' }}>
                  {showPass ? '🙈' : '👁'}
                </button>
              </div>
            </div>
    ...
    >
      ...
      {!p.comingSoon && (
        <div style={{ paddingTop: 14, borderTop: `1px dashed ${T.border}`, display: 'flex', gap: 8 }}>
          <button
            onClick={e => { e.stopPropagation(); if (active) window.open(p.url, '_blank') }}
            disabled={!active}
            style={{
...
function HubPage({ user, subscriptions, onLogout }) {
  ...
  function handleSelect(key) { window.open(PRODUCTS[key].url, '_blank') }
